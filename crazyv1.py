...
class CrazyTimeAnalyzer:
  ...
  def analyze_sequence_transitions(self, df: pd.DataFrame, current_sector: str):
    sectors = df["wheelSector"].values
    next_after_current = []

    for i in range(len(sectors) - 1, 0, -1):
      if sectors[i] == current_sector:
        next_after_current.append(sectors[i - 1])

    if not next_after_current:
      return {}

    total = len(next_after_current)
    counts = pd.Series(next_after_current).value_counts()
  ...
  def calculate_potency(self, df: pd.DataFrame):
    ...
    latest_sector = df.iloc[0]["wheelSector"]
    transition_probs = self.analyze_sequence_transitions(df, latest_sector)
    metrics = {}
    ...
    for sector, expected_gap in EXPECTED_GAPS.items():
      ...
      # Teto para limitar o peso de atrasos longos
      ...
      metrics[sector] = {
          ...
          f"Pós-[{latest_sector}]": f"{(observed_trans_prob * 100):.1f}%",
  ...
  def analyze_patterns(self):
    ...
    potency_df = self.calculate_potency(df)
